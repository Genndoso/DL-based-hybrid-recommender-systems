...
class RatingDataset(Dataset):
    def __init__(self, input_tensor, target_tensor):#, holdout_tensor):
        self.input_tensor = input_tensor.long()
        self.target_tensor = target_tensor.long()
        #self.holdout_tensor = holdout_tensor.long()

    def __getitem__(self, index):
        return self.input_tensor[index], self.target_tensor[index]#, self.holdout_tensor[index]
...
def torchify_holdout(holdout):
    pass

def prepare_loader(train_df, test_df, feature_embeddings, target, validate=None):
    """
    process our data and return train and validate dataloaders
    """
    if validate is not None:
        # if we are validating, create a loader of the current user_id (with their user info)
